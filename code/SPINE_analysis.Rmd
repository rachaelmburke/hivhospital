...
## 2. Stacked barchart -- supplementary figure 1
...
spine %>% 
  ...
  geom_text(data=label_sum, aes(x=ymd("2016-10-01"), y=0.95, 
...
ggsave(here("figures/Sfig_1.pdf"), width = 9, height = 6, dpi=300)
...
fig_extra2 <- spine %>% 
  ...
  mutate(number=n()) %>%
  ggplot() +
  geom_bar(aes(fill=hiv,x=quarter), position="stack") +
  geom_text(data=label_sum, aes(x=ymd("2016-10-01"), y=220, 
                label=paste0("n=",sum, sep=""))) +
  facet_grid(sex ~ageg10) + 
  scale_fill_manual(values=wes_palette("BottleRocket2")) +
    labs(
      ...
      #subtitle = "By quarter, and sex. n = total numer admissions between Q3.2012 to Q3.2019",
       x="Year and quarter",
       y="Number of admissions by HIV status",
       fill="HIV status") +
  scale_y_continuous() +
  scale_x_date(breaks=breaks1, date_labels = "%b %Y") +
  theme_bw() +
...
ggsave(here("figures for Blantyre grand round presentation/fig_extra2.pdf"), width = 9, height = 6, dpi=300)
```
...
spine_gp <- spine %>%
  group_by(quarter, ageg10, sex, hiv) %>% #NB. not interested in 'outcome' for this bit, so not grouped by this
  summarise(n_observed=n())

both_gp <- left_join(spine_gp, cens_edit) %>%
  mutate(n = case_when(is.na(n_observed) ~ 0L,
                       TRUE ~ n_observed)) %>%  # 'n' is now from n_imputed
  mutate(inc = (n/pop) * 100000) %>%
  ungroup() %>%
  mutate(q = group_indices(.,quarter))

# Checking highest and lowest quarters of incidence admissions
y <- both_gp %>% group_by(hiv,quarter) %>% summarise(n=sum(n_observed), pop=sum(pop)) %>% mutate(inc=(n / pop) *100000)
y %>% filter(hiv=="Positive") %>% arrange(-inc)

z <- both_imp_gp %>% group_by(hiv,quarter, .imp) %>% summarise(n=sum(n_imputed), pop=sum(pop)) %>% 
  group_by(hiv,quarter) %>% summarise(n=mean(n), pop=mean(pop)) %>% mutate(inc=(n / pop) *100000)
z %>% filter(hiv=="Positive") %>% arrange(-inc)

```
```{r, include=FALSE}
# Drawing pie charts for a slide in talk explaining multiple imputation
a <- spine %>% 
  mutate(hiv=as.character(hiv)) %>%
  mutate(hiv=replace_na(hiv,"Unknown")) %>%
  mutate(hiv=factor(hiv, levels=c("Unknown", "Negative","Positive"))) %>%
...
pie(a$number, labels=a$hiv, col=wes_palette("BottleRocket2"), main="Original Observations")
...
for(i in c(1:25)){
  pie(b$data[[i]]$number, labels=b$data[[i]]$hiv, col=c("#CB2314", "#273046"))
}

```

```{r}
# So this is mean of all 25 imputations (to use for plotting points in graph)
both_imp_gp2 <- both_imp_gp %>% group_by(quarter, ageg10, sex, hiv, q) %>%
  summarise(n_imputed=mean(n_imputed), n=mean(n), pop=mean(pop)) %>% mutate(inc=(n_imputed/pop)*100000)

# usethis::use_data(both_imp_gp, overwrite = TRUE) # saving data
label_sum2 <- spine_imp %>%
  group_by(ageg10, sex, hiv, .imp) %>%
  summarise(sum = sum(n())) %>%
  group_by(ageg10, sex, hiv) %>%
  summarise(sum = mean(sum)) %>%
  mutate(sum=round(sum)) %>%
  filter(hiv=="Positive")

fig_extra7 <- both_imp_gp2 %>% 
  filter(hiv=="Positive") %>%
  ggplot() +
  geom_point(aes(y=inc,x=quarter), position="stack", size=0.3) +
  geom_text(data=label_sum2, aes(x=ymd("2016-10-01"), y=250, 
                label=paste0("n=",sum, sep=""))) +
  facet_grid(sex ~ageg10) + 
  scale_fill_manual(values=wes_palette("BottleRocket2")) +
    labs(
...
ggsave(here("figures for Blantyre grand round presentation/fig_extra7.pdf"), width = 9, height = 6, dpi=300)

fig_extra8 <- both_imp_gp %>% 
  filter(hiv=="Positive") %>%
  mutate(.imp=as.character(.imp)) %>%
  ggplot() +
  geom_point(aes(y=inc,x=quarter, color=.imp), size=0.3) +
  facet_grid(sex ~ageg10) + 
  scale_fill_manual(values=wes_palette("BottleRocket2")) +
    labs(
      #title="HIV status of adults admitted to QECH",
...
fig_extra8
ggsave(here("figures for Blantyre grand round presentation/fig_extra8.pdf"), width = 9, height = 6, dpi=300)
...
Dots are from mean of 25 imputed dataframes, model predictions are from melding across all dataframes.
...
ggplot() +
  ...
  geom_point(data=both_imp_gp2 %>% filter(hiv=="Positive"), aes(x=quarter, y=((n_imputed/pop)*100000), color=hiv),size=0.1) +
  geom_point(data=both_imp_gp2 %>% filter(hiv=="Negative"), aes(x=quarter, y=((n_imputed/pop)*100000), color=hiv),size=0.1) +
  facet_grid(sex ~ ageg10) +
  scale_x_date(breaks=breaks1, date_labels = "%b%Y ") +
...
## 11. Secondary objective -- outcome (dead or alive) Table of outcomes (table 3)
